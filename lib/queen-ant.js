...
QueenAnt.prototype = {
  ...
  removeWorkerAnt: function(socket) {
    ...
    logger.info('workerAnt ' + workerAntToRemove.id + ' disconnected');
  },
  ...
  setTaskState: function(taskId, taskState) {
    kue.Job.get(taskId, function (err, job) {
      if (err) {
        ...
        logger.info('Task ' + job.id + ' state changed  to : ' + taskState);
      }
    });
  },
  ...
  setTaskPriority: function(taskId, taskPriority) {
    kue.Job.get(taskId, function (err, job) {
      if (err) {
        ...
        logger.info('Task ' + job.id + ' priority changed  to : ' + taskPriority);
      }
    });
  },
