...
function QueenAnt(host, port) {
  ...
  this.onTaskCompletedCallbacks = [];
  this.onTaskProcessededCallback = null;
}
...
QueenAnt.prototype = {
  ...
  createServer: function() {
    ...
    net.createServer(function (socket) {
      ...
      socket.on('message', function (message) {
        ...
        switch(message.type) {
        case 'PROCESSING':
          self.onTaskProcessededCallback(message.taskObjId);
          break;
        case 'SUCCESS':
          kue.Job.get(parseInt(message.taskId, 10), function (err, job) {
            ...
            _.first(_.where(self.onTaskCompletedCallbacks, {
              'taskType': job.type
  ...
  addTask: function(task, callback) {
    ...
    job
    .on('enqueue', function() {
      logger.info(colors.yellow('task', job.id, 'enqueued', JSON.stringify(job.data.task)));
    })
    .on('complete', function() {
      logger.info(colors.green('task', job.id, 'completed', JSON.stringify(job.data.task)));
    })
    .on('failed', function() {
      logger.info(colors.red('task', job.id, 'failed', JSON.stringify(job.data.task)));
    })
  ...
  setTaskState: function(taskId, taskState) {
    kue.Job.get(taskId, function (err, job) {
      if (err) {
        ...
        logger.info(colors.gray('Task ' + job.id + ' state changed  to : ' + taskState));
      }
    });
  },
  ...
  setTaskPriority: function(taskId, taskPriority) {
    kue.Job.get(taskId, function (err, job) {
      if (err) {
        ...
        logger.info(colors.gray('Task ' + job.id + ' priority changed  to : ' + taskPriority));
      }
    });
  },
  ...
  onTaskCompleted: function(callbacks) {
    var self = this;
    callbacks.forEach(function(callback) {
      callback = callback || function() {};
      self.onTaskCompletedCallbacks.push(callback);
    });
  },

  // To set Task processing callback
  onTaskProcessed: function(callback) {
    callback = callback || function() {};
    this.onTaskProcessededCallback = callback;
  },
};
