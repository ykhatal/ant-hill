...
QueenAnt.prototype = {
    ...
    net.createServer(function(socket) {
      ...
      socket.on('message', function(message) {
        ...
        case 'SUCCESS':
          kue.Job.get(parseInt(message.taskId), function (err, job) {
            if (err) {
              ...
              _.first(_.where(self.callbacks, {
                'taskType': job.type
        ...
        case 'CONNECTED':
          ...
          break;
        case 'DISCONNECTED':
          self.setTaskState(parseInt(message.taskId), 'inactive');
          self.setTaskPriority(parseInt(message.taskId), 'medium');
          break;
        }
      });
  ...
  addTask: function(task) {
    ...
    job.on('enqueue', function() {
      logger.info(colors.bgWhite.gray('task', job.id, 'enqueued', JSON.stringify(job.data.task)));
    });
    job.on('complete', function() {
      logger.info(colors.bgWhite.gray('task', job.id, 'completed', JSON.stringify(job.data.task)));
    });
    job.on('failed', function() {
      logger.info(colors.bgWhite.gray('task', job.id, 'failed', JSON.stringify(job.data.task)));
    });
  },
