...
QueenAnt.prototype = {
  ...
  addTask: function(task, callback) {
    ...
    job
    .on('enqueue', function() {
      logger.info(colors.bgWhite.yellow('task', job.data.taskId, 'enqueued', JSON.stringify(job.data.task)));
    })
    .on('complete', function() {
      logger.info(colors.bgWhite.green('task', job.data.taskId, 'completed', JSON.stringify(job.data.task)));
    })
    .on('failed', function() {
      logger.info(colors.bgWhite.red('task', job.data.taskId, 'failed', JSON.stringify(job.data.task)));
    })
