...
function QueenAnt(host, port) {
  ...
  this.onTaskProccessCallback = null;
}
...
QueenAnt.prototype = {
  ...
  createServer: function() {
    ...
    net.createServer(function(socket) {
      ...
      socket.on('message', function(message) {
        ...
        switch(message.type) {
        case 'PROCCESSING':
          self.onTaskProccessCallback(parseInt(message.taskId, 10));
          break;
        case 'SUCCESS':
  ...
  // To set Task proccessing callback
  onTaskProccess: function(callback) {
    callback = callback || function() {};
    this.onTaskProccessCallback = callback;
  },
