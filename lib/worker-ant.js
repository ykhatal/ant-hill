...
WorkerAnt.prototype = {
  ...
  connect: function() {
    ...
    self.socket.on('connect', function() {
      self.state = self.antStates.CONNECTED;
      if (self.intervalId && self.state === self.antStates.CONNECTED) {
        clearInterval(self.intervalId);
        self.intervalId = null;
      }
      self.talkToQueenAnt({ type: 'MESSAGE', state: self.state, data: 'Worker ' + self.state });
  ...
  startWork: function() {
    ...
    self.tasksQueue.process(self.taskType, function (job, done, ctx) {
      ...
      self.callback(job, function(result) {
        ...
        self.state = self.antStates.READY;
        self.talkToQueenAnt({ type: 'MESSAGE', state: self.state, data: result, taskId: job.id });
      });
    });
  }
};
