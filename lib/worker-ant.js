...
WorkerAnt.prototype = {
  ...
  startWork: function() {
    ...
    self.tasksQueue.process(self.taskType, function (job, done, ctx) {
      ...
      logger.info(colors.bgWhite.gray('Doing task ' + job.id));
      self.setState(self.antStates.BUSY);
      self.done = done;
      self.talkToQueenAnt({ type: 'PROCESSING', workerAntState: self.state, taskId: job.id });
  ...
  doBeforeShutdown: function(callback) {
    ...
    var taskId = (this.currentTask) ? this.currentTask.id : -1;
