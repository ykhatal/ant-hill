...
WorkerAnt.prototype = {
  ...
  startWork: function() {
    ...
    self.tasksQueue.process(self.taskType, function (job, done, ctx) {
      ...
      logger.info(colors.bgWhite.gray('Doing task ' + job.data.taskId));
      self.setState(self.antStates.BUSY);
      self.done = done;
      self.talkToQueenAnt({ type: 'PROCESSING', workerAntState: self.state, taskId: job.data.taskId });
      self.callback(job, function(err, result) {
        self.setState(self.antStates.READY);
        if (err) {
          self.talkToQueenAnt({ type: 'ERROR', workerAntState: self.state, taskId: job.data.taskId, error: err });
        } else {
          self.talkToQueenAnt({ type: 'SUCCESS', workerAntState: self.state, taskId: job.data.taskId, success: result });
        }
  ...
  setTaskState: function(task, toState) {
    ...
    logger.info('Set job[' + task.data.taskId + '] state from: ' + task._state + ' to : ' + toState);
  },
  ...
  doBeforeShutdown: function(callback) {
    ...
    var taskId = (this.currentTask) ? this.currentTask.data.taskId : -1;
    this.talkToQueenAnt({ type: 'DISCONNECTED', workerAntState: this.state, taskId: taskId });
    callback();
